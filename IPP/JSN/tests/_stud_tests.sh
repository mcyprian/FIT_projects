...
while [ $num -le 16 ]; do
    ...
    for suffix in "!!!" "err"; do
        diff_output=$(diff $test_name.$suffix ref-out/$test_name.$suffix)
        if [ -z "$diff_output" ]; then
            echo "$test_name.$suffix: OK" >> $output
        else
            echo "$test_name.$suffix: FAILED:" >> $output
            echo "$diff_output" >> $output
        fi
    
    done
    java -jar /pub/courses/ipp/jexamxml/jexamxml.jar $test_name.xml ref-out/$test_name.xml delta.xml /pub/courses/ipp/jexamxml/jsn_options
    ret="$?"
    if [ $ret -eq 0 ]; then
        echo "$test_name.xml: OK" >> $output
    else
        echo "$test_name.xml: FAILED" >> $output
        cat $test_name.xml.log >> $output
    fi
    num=$((num+1))
done
